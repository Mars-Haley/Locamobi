...
function populateVehicleGrid() {
  ...
  vehicles.forEach((vehicle) => {
    ...
    card.className =
      "bg-white rounded-2xl shadow-soft overflow-hidden card-hover";
    card.innerHTML = `
            ...
            <div class="p-6">
                ...
                <div class="flex space-x-2">
                    <button onclick="viewDetails(${
                      vehicle.id
                    })" class="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-800 py-2 px-4 rounded-lg font-medium transition-colors">
                        Ver detalhes →
                    </button>
                    <button onclick="rentNow(${
                      vehicle.id
...
function initializeRegistrationFormListeners() {
  ...
  if (currentStepElement) {
    ...
    const prevButton = currentStepElement.querySelector(
      '[onclick="prevStep()"]'
    );
...
function handleFiles(event) {
  ...
  files.forEach((file) => {
    ...
    fileElement.className =
      "flex items-center justify-between bg-gray-50 p-3 rounded-lg mb-2";
